...
describe('eight action builders (happy path)', () => {
  ...
  it("action 4 qualified lead -> 'lead.qualified'", () => {
  ...
  it('action 7 consent -> one of the three consent event names', () => {
    for (const state of ['granted', 'withdrawn', 'policy_updated'] as const) {
      ...
      expectEnvelope(event, `consent.${state}`);
...
describe('rejection matrix (required fields throw TypeError)', () => {
  it("qualified lead without lead id -> TypeError 'lead.qualified.lead_id'", () => {
    expect(() => buildQualifiedLeadEvent({}, {}, meta)).toThrow(TypeError);
    expect(() => buildQualifiedLeadEvent({}, {}, meta)).toThrow(/lead\.qualified\.lead_id/);
    expect(() => buildQualifiedLeadEvent({ 'Lead ID': '   ' }, {}, meta)).toThrow(/lead\.qualified\.lead_id/);
  });

  it('purchase without transaction id -> TypeError sale.recorded.transaction_id', () => {
    expect(() => buildPurchaseEvent({ value: 100, currency: 'EUR' }, {}, meta)).toThrow(/sale\.recorded\.transaction_id/);
  });
  ...
  it('purchase without currency -> TypeError sale.recorded.currency', () => {
    ...
    ).toThrow(/sale\.recorded\.currency/);
  });
});
