...
function baseEvent(eventName: string, meta: BuildMeta): BlockEvent {
...
export function buildQualifiedLeadEvent(
  ...
  const leadId = requireField('lead.qualified', 'lead_id', merged['lead_id']);
...
export function buildPurchaseEvent(
  ...
  const transactionId = requireField('sale.recorded', 'transaction_id', input.transactionId);

  const valueRaw =
    typeof input.value === 'number' ? input.value : Number(String(input.value ?? '').trim());
  if (!Number.isFinite(valueRaw) || valueRaw <= 0) {
    throw new TypeError('sale.recorded.value is required and must be a positive finite number');
  }

  const currency = requireField('sale.recorded', 'currency', input.currency).toUpperCase();
