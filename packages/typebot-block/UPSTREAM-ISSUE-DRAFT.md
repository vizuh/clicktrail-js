...
A working reference implementation exists (`@clicktrail/typebot-block`, MIT): pure `(mappedVariables, config) -> event` builders, injected send/clock, vitest-covered. Lifting it into `packages/blocks/clicktrail/` means swapping the plain config object for `option` helpers and registering the typed block schema — no logic changes expected.
