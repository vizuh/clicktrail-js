...
```ts
import { createClickTrailBlock } from '@clicktrail/typebot-block';
...
```sh
pnpm install --filter @clicktrail/typebot-block
pnpm --filter @clicktrail/typebot-block build   # tsc -p tsconfig.build.json
pnpm --filter @clicktrail/typebot-block test    # vitest
pnpm --filter @clicktrail/typebot-block exec tsc -p tsconfig.json --noEmit
```
