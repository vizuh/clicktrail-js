...
const CONTRACT: ReadonlyArray<{
  ...
  { displayName: 'Attach Attribution', pieceName: 'attach_attribution', sample: () => buildAttachAttribution({}), expectedEventName: 'lead.attribution_attached' },
  { displayName: 'Record Booking', pieceName: 'record_booking', sample: () => buildRecordBooking({}), expectedEventName: 'booking' },
  { displayName: 'Record Qualified Lead', pieceName: 'record_qualified_lead', sample: () => buildQualifiedLead({ leadId: 'L' }), expectedEventName: 'lead.qualified' },
  { displayName: 'Record Sale', pieceName: 'record_sale', sample: () => buildRecordSale({ transactionId: 'T', value: 1, currency: 'EUR' }), expectedEventName: 'sale.recorded' },
  { displayName: 'Record Refund', pieceName: 'record_refund', sample: () => buildRecordRefund({ originalTransactionId: 'T' }), expectedEventName: 'refund.issued' },
];

const CONSENT_STATES = ['consent.granted', 'consent.withdrawn', 'consent.policy_updated'] as const;
