 ...
 * outside the node. Validation mirrors the @clicktrail/astro server
...
async function build(
  ...
  return buildEventPayload(payload, eventName, data, context);
}
...
export async function buildLeadCreateOrIdentify(
  ...
  return build('lead', payload, data, mergedContext);
}
...
/** 'lead.attribution_attached' — attach a trail to an existing lead. */
export async function buildAttachAttribution(
  ...
  return build('lead.attribution_attached', merged, data, context);
}
...
export async function buildUpdateStage(
  ...
  return build('lead.stage_updated', {}, data, context);
}
...
export async function buildMarkQualified(
  ...
  return build('lead.qualified', {}, data, context);
}
...
export async function buildMergeVisitor(
  ...
  return build('lead.merged', {}, data, context);
}
...
/** 'appointment.booked' — a booked appointment. Money fields are validated when present. */
export async function buildRecordAppointment(
  ...
  return build('appointment.booked', {}, data, context);
}
...
/** 'appointment.completed' — an appointment that actually happened. */
export async function buildRecordCompletedAppointment(
  ...
  return build('appointment.completed', {}, data, context);
}
...
/** 'sale.recorded' — a closed-won purchase. transactionId/value/currency required. */
export async function buildRecordSale(
  ...
  return build('sale.recorded', {}, data, context);
}
...
/** 'revenue.recurring' — subscription revenue recognition. */
export async function buildRecordRecurringRevenue(
  ...
  return build('revenue.recurring', {}, data, context);
}
...
/** 'refund.issued' — refund against an original sale; negative-safe value. */
export async function buildRecordRefund(
  ...
  return build('refund.issued', {}, data, context);
}
...
/** 'offline_conversion.sent' — GCLID-style offline conversion upload. clickId OR trailId required. */
export async function buildSendOfflineConversion(
  ...
  return build('offline_conversion.sent', {}, data, context);
}
...
/** 'consent.granted' — record a consent decision. */
export async function buildRecordConsent(
  ...
  return build('consent.granted', {}, data, context);
}
...
/** 'consent.withdrawn' — record a consent withdrawal. */
export async function buildRecordWithdrawal(
  ...
  return build('consent.withdrawn', {}, data, context);
}
...
/** 'consent.policy_updated' — announce a new consent policy version. */
export async function buildUpdateConsentPolicy(
  ...
  return build('consent.policy_updated', {}, data, context);
}
...
/**
 * 'visitor.anonymized' — emits a deletion REQUEST event. Actual erasure
...
export async function buildAnonymizeVisitor(
  ...
  return build('visitor.anonymized', {}, { ...data }, { ...context, identity: { visitorId } });
}
...
export const OPERATIONS: Readonly<Record<ResourceName, Readonly<Record<string, OperationDef>>>> = Object.freeze({
  lead: Object.freeze({
    createOrIdentify: op('lead', buildLeadCreateOrIdentify),
    attachAttribution: op('lead.attribution_attached', buildAttachAttribution),
    updateStage: op('lead.stage_updated', buildUpdateStage),
    markQualified: op('lead.qualified', buildMarkQualified),
    mergeVisitor: op('lead.merged', buildMergeVisitor),
  }),
  conversion: Object.freeze({
    recordAppointment: op('appointment.booked', buildRecordAppointment),
    recordCompletedAppointment: op('appointment.completed', buildRecordCompletedAppointment),
    recordSale: op('sale.recorded', buildRecordSale),
    recordRecurringRevenue: op('revenue.recurring', buildRecordRecurringRevenue),
    recordRefund: op('refund.issued', buildRecordRefund),
    sendOfflineConversion: op('offline_conversion.sent', buildSendOfflineConversion),
  }),
  consent: Object.freeze({
    recordConsent: op('consent.granted', buildRecordConsent),
    recordWithdrawal: op('consent.withdrawn', buildRecordWithdrawal),
    updateConsentPolicy: op('consent.policy_updated', buildUpdateConsentPolicy),
    anonymizeVisitor: op('visitor.anonymized', buildAnonymizeVisitor),
  }),
});
