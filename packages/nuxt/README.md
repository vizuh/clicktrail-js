...
```sh
npx nuxi module add @clicktrail/nuxt
# or
npm install @clicktrail/nuxt
```
...
| Requirement | How |
|---|---|
| Browser SDK injection | The module registers `@clicktrail/nuxt/plugin` as a client plugin; SSR renders stay side-effect free. |
...
| Server-side conversions | `@clicktrail/nuxt/server` exports `ClickTrailServer` with `trackLead`, `trackBooking`, `trackPurchase`. |
...
```vue
<script setup>
import { useClicktrail } from '@clicktrail/nuxt/composable';
...
```ts
import { ClickTrailServer, parseIdentityFromCookies } from '@clicktrail/nuxt/server';
...
`@clicktrail/nuxt/nitro` exports `createEventHandler(config, fetch)` — the same validated forwarding handler the injected route uses (415/413/400/502 matrix, IP headers never forwarded, allowlisted `forwardHeaders`) — plus `parseIdentityFromCookies`.
...
- [`@clicktrail/astro`](https://www.npmjs.com/package/@clicktrail/astro) — the same engine for Astro.
