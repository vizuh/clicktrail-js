...
| Requirement | How |
|---|---|
| Browser SDK injection | A `page` pattern script imports `@clicktrail/astro/client` into every page. |
...
| Server-side conversions | `@clicktrail/astro/server` exports `ClickTrailServer` with `trackLead`, `trackBooking`, `trackPurchase`. |
