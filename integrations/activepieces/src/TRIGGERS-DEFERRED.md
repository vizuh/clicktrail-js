...
`POST <collector>/webhooks` API in `onEnable`, filter `event_name === 'lead'`,
hand `payload.events[0]` to the flow in `run`, deregister in `onDisable`.

## 2. Conversion Recorded (`conversion.recorded`)

Fires when a booking/sale/refund event lands for a trail.

Sketch: same webhook-trigger skeleton as New Lead; filter on
`marketing_trail.event_name ∈ {'booking', 'sale.recorded', 'refund.issued'}`
and pass the stamped event through.

## 3. Consent Changed (`consent.changed`)

Fires when a visitor's consent state changes.

Sketch: same webhook-trigger skeleton as New Lead; filter on
`marketing_trail.event_name ∈ {'consent.granted', 'consent.withdrawn',
'consent.policy_updated'}` and pass the stamped event through.
