/**
 * @vizuh/clicktrail-nuxt/server — server-side conversion helpers.
 *
 * Typed builders + senders for the three conversion classes ClickTrail
 * treats as first-class outcomes: leads, bookings, purchases. Identity
 * comes from the visitor's own first-party cookies (the `attribution`
 * payload cookie + `ct_session` state written by the browser SDK), read in
 * a Nuxt server route / middleware via the request's Cookie header.
