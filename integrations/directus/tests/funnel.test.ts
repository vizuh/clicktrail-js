...
describe('aggregateFunnel', () => {
  ...
  it('falls back to (direct) when campaign is missing or blank', () => {
    const summary = aggregateFunnel([row('lead_created'), row('lead_created'), { event_name: 'sale', campaign: '   ' }]);
  ...
  it('sorts multi-campaign output by leads desc then campaign asc', () => {
    const summary = aggregateFunnel([
      ...Array.from({ length: 3 }, () => row('lead_created', 'zeta')),
      ...Array.from({ length: 3 }, () => row('lead_created', 'alpha')),
      row('lead_created', 'beta'),
      row('lead_qualified', 'alpha'),
      row('sale', 'alpha'),
    ]);
