...
export function createWebPixelScript({ eventName = 'page_viewed', cookieName = 'ct_attribution' } = {}) {
  ...
  return `analytics.subscribe(${escapeUnsafeChars(JSON.stringify(eventName))}, async (event) => { const url = new URL(event.context.document.location.href); const ids = {}; for (const key of ${escapeUnsafeChars(JSON.stringify(KEYS))}) { const value = url.searchParams.get(key); if (value) ids[key] = value.slice(0, 512); } if (Object.keys(ids).length) await browser.cookie.set(${escapeUnsafeChars(JSON.stringify(cookieName))} + '=' + encodeURIComponent(JSON.stringify(ids)) + '; Path=/; SameSite=Lax; Secure'); });`;
}
