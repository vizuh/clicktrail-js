...
| 6 | WP parity fixture capture from live click-trail-handler plugin behavior | 2 gate | PREP DONE | Static-analysis stage complete: 23 draft fixtures + docs/WP-PARITY-DRAFT.md with SUPERVISOR RULINGS; live-runtime validation still queued |
...
| 11 | Core parity fixes per WP-PARITY-DRAFT.md SUPERVISOR RULINGS: symmetric related-hosts, case-insensitive keys, last-wins params, macro rejection, ft-guard counts click IDs, canonical referrer names, intl TLD search rules, channel-label layer (ft_channel/lt_channel), ms timestamps, landing=full href, harness `stored` support; re-rule affected goldens/drafts | 2 prep | IN PROGRESS (worker: core-parity-fixer) | Classifier semantics change -> CLASSIFIER_VERSION bump discussion required |
