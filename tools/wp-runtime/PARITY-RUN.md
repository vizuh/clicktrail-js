...
in a Node `vm` sandbox against all 23 draft fixtures, then ran the TS engine
...
|---|---|
| MATCH | 14 |
| RULED DIFF (expected per supervisor ruling) | 8 |
| NEW FINDING (unexplained diff — investigate) | 1 |
| HARNESS ERROR | 0 |
...
| Field | Plugin | Engine |
|---|---|---|
| ft_gclid | `"EAIaIQobChMTest"` | `"(absent)"` |
| ft_medium | `"(absent)"` | `"cpc"` |
| ft_source | `"(absent)"` | `"google"` |
| lt_gclid | `"EAIaIQobChMTest"` | `"(absent)"` |
| lt_medium | `"(absent)"` | `"cpc"` |
...
- **wp-fbclid-only-organic-facebook**: `fbc` — plugin=`"fb.1.1787481000000.IwARabc123"`, engine=`"(absent)"`
