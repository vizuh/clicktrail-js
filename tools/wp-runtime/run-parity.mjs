...
function bundleEngine() {
  ...
  writeFileSync(entry, [
    `export { parseAttributionUrl } from ${JSON.stringify(join(ROOT, 'packages/core/src/core/parse.ts'))};`,
    `export { emptyAttribution, mergeAttributionTouch } from ${JSON.stringify(join(ROOT, 'packages/core/src/core/merge.ts'))};`,
    '',
  ].join('\n'));
...
async function main() {
  ...
  lines.push(`(esbuild-bundled from \`packages/core/src/core\`) on the same inputs.`);
